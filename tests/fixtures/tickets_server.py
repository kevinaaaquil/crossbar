"""A support-desk MCP server used as the integration surface across the suite.

Real subprocess, real JSON-RPC over real pipes. Read-only tools advertise
`readOnlyHint` so the connector's probe classification can be tested against
annotations rather than guesswork.

Run as: ``python -m tests.fixtures.tickets_server``
"""
